...
Compile with:
$ gcc -Wall  -fPIC -shared -Wl,-soname,glWrap.so -ldl -lglut -o glWrap.so  glWrap.c

Run with:
$ LD_PRELOAD=../glWrap/glWrap.so <OpenGL application here>
                                    ...
                                    const GLvoid *pixels )
{
    ...
    if (real_glTexImage2D == NULL) {
        fprintf(stderr, "glWrap: wrapping glTexImage2D...\n");
        real_glTexImage2D = dlsym(RTLD_NEXT, "glTexImage2D");
        char *errmsg = dlerror();
        if (errmsg  != NULL) {
            fprintf(stderr, "glWrap: dlopen failed : %s\n", errmsg);
            exit(1);
        } else {
            fprintf(stderr, "glWrap: wrapping done, textures now clamped to %dx%d\n", max_texture_size, max_texture_size);
        }
    }
    ...
    {
        printf("glTexImage2D: level %d, size=%dx%d%s\n"
        ...
        if( pixels != NULL && target != GL_PROXY_TEXTURE_2D )
        {
            ...
            if(e) fprintf(stderr, "glWrap: Error #%d in gluScaleImage(): %s\n",e,gluErrorString(e));
            pixels = smallImage;
        }

        width = newWidth;
        height = newHeight;
        printf("glTexImage2D: new size=%dx%d%s\n",width, height, border?", with border":"");
    }
    real_glTexImage2D( target, level,
                        ...
                        pixels );
    int e = glGetError();
    if(e!=GL_NO_ERROR) fprintf(stderr, "glWrap: Error #%d after glTexImage2D(): %s\n",e,gluErrorString(e));
}
